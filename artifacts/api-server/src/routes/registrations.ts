...
router.post("/matches/:id/register", requireAuth as any, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (match.status !== "upcoming") {
    res.status(400).json({ error: "Registration is closed for this match" });
    return;
  }
  ...
  const existing = await db
    ...
    .where(and(
      eq(registrationsTable.matchId, params.data.id),
      eq(registrationsTable.playerId, playerId)
    ));

  if (existing.length > 0) {
    res.status(409).json({ error: "Already registered for this match" });
    return;
  }
  ...
  if (currentCount >= match.maxPlayers) {
    res.status(400).json({ error: "Match is full" });
    return;
  }
  ...
  const [player] = await db
    .select({ username: playersTable.username, gameId: playersTable.gameId })
    .from(playersTable)
...
router.get("/my-registrations", requireAuth as any, async (req: AuthRequest, res): Promise<void> => {
  ...
  const regsWithMatch = await Promise.all(
    regs.map(async (reg) => {
      const [match] = await db
        .select()
        .from(matchesTable)
        .where(eq(matchesTable.id, reg.matchId));
